...
# Toolchain containing compiler and MPI
proc mpiToolchain {
...
# Toolchain containing compiler
proc compilerToolchain {
    if {! [module-info mode remove] } {
        # If not in remove mode get the compiler and MPI version
        if {! [ is-loaded intel-cc intel-fc gcc ]} {
            puts stderr "ERROR: No compiler is loaded, unable to load library"
        ...
        set prefix "$root/$name/$ver/$compiler"

        if {! [ file exists $prefix/.build_succeeded ] } {
            puts stderr "ERROR: [module-info version [module-info name]] has not been compiled for $compiler"
            break
        }
    }
    createEnv $prefix
...
# Toolchain without alternatives (e.g. binary-only)
proc basicToolchain {
    set prefix "$root/$name/$ver/$compiler"
    createEnv $prefix
}

# Set the environment variables after finding the prefix
proc createEnv {prefix} {
    # Don't allow multiple versions of the same library
    conflict $name
